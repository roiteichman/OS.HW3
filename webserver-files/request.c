...
*/
  /*
   void requestError(int fd, char *cause, char *errnum, char *shortmsg, char *longmsg, struct stats *stats)
{
   char buf[MAXLINE], body[MAXBUF];
...
void requestHandle(int fd, struct timeval* arrival, struct timeval* dispatch, long id, int* static_counter, int* dynamic_counter, int* total_counter)
{
   ...
   //increasing the total counter of request handled
   (*total_counter)++;

   if (strcasecmp(method, "GET")) {
      requestError(fd, method, "501", "Not Implemented", "OS-HW3 Server does not implement this method",
                   arrival, dispatch, id, static_counter, dynamic_counter, total_counter);
      return;
   }
   requestReadhdrs(&rio);
